...
!!! info
    The style guide is always a work in progress, and not all PythonStyleDicts code
    follows the rules. When modifying PythonStyleDicts, please fix the style violations
...
PythonStyleDicts uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/PythonStyleDicts.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the PythonStyleDicts directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to PythonStyleDicts have
